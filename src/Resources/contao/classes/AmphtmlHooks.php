    ...
    public function ampGetPageLayout($objPage, &$objLayout, $objPageRegular)
    {
        ...
        if($amphtml == 1) {
            if ($amphtml && ($page = \PageModel::findByPk($objPage->id)) !== null) {
                $arrSearch = array("?amp", "&amp");
                $strUrl = str_replace($arrSearch, "", "http" . (($_SERVER['SERVER_PORT'] == 443) ? "s://" : "://")."{$_SERVER['HTTP_HOST']}{$_SERVER['REQUEST_URI']}");
                // @ToDo find contao way with generateFrontendUrl
